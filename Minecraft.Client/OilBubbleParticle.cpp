...
OilBubbleParticle::OilBubbleParticle(Level* level, double x, double y, double z, double xa, double ya, double za) : Particle(level, x, y, z, xa, ya, za)
{
    rCol = 1.0f;
    gCol = 1.0f;
    bCol = 1.0f;
...
void OilBubbleParticle::tick()
{
    ...
    if (level->getMaterial(Mth::floor(x), Mth::floor(y), Mth::floor(z)) != Material::oil) remove();
