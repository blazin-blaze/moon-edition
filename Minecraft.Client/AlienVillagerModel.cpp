...
void AlienVillagerModel::_init(float g, float yOffset, int xTexSize, int yTexSize)
{
	head = (new ModelPart(this))->setTexSize(xTexSize, yTexSize);
	head->setPos(0, 0 + yOffset, 0);
	head->texOffs(0, 0)->addBox(-4, -10, -4, 8, 10, 8, g);
	head->texOffs(24, 43)->addBox(-5.0F, -17.0F, -5.0F, 10.0F, 10.0F, 10.0F); //new head
	...
	body->texOffs(0,34)->addBox(-4, 0, -3, 8, 12, 6, g);
	body->texOffs(36,0)->addBox(-4, 0, -3, 8, 18, 6, g + 0.5f);

	arms = (new ModelPart(this))->setTexSize(xTexSize, yTexSize);
	arms->setPos(0, 0 + yOffset + 2, 0);
	arms->texOffs(16,22)->addBox(-6 - 2, -2, -2, 4, 8, 4, g);
	arms->texOffs(16,22)->addBox(6 - 2, -2, -2, 4, 8, 4, g);
	arms->texOffs(0,52)->addBox(-4, 2, -2, 8, 4, 4, g);

	leg0 = (new ModelPart(this, 0, 18))->setTexSize(xTexSize, yTexSize);
	leg0->setPos(-2, 12 + yOffset, 0);
	leg0->addBox(-2, 0, -2, 4, 12, 4, g); // Leg0

	leg1 = (new ModelPart(this, 0, 18))->setTexSize(xTexSize, yTexSize);
	leg1->bMirror = true;
...
AlienVillagerModel::AlienVillagerModel(float g) : Model()
{
	_init(g, 0, 64, 64);
}

AlienVillagerModel::AlienVillagerModel(float g, float yOffset, int xTexSize, int yTexSize) : Model()
{
	_init(g, yOffset, xTexSize, yTexSize);
}

void AlienVillagerModel::render(shared_ptr<Entity> entity, float time, float r, float bob, float yRot, float xRot, float scale, bool usecompiled)
{
...
void AlienVillagerModel::setupAnim(float time, float r, float bob, float yRot, float xRot, float scale, shared_ptr<Entity> entity, unsigned int uiBitmaskOverrideAnim)
{
