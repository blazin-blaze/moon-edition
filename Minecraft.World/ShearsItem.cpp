...
{
	if (tile == Tile::leaves_Id || tile == Tile::web_Id || tile == Tile::greenWeb_Id || tile == Tile::tallgrass_Id || tile == Tile::vine_Id || tile == Tile::tripWire_Id)
	{
...
{
	return tile->id == Tile::web_Id || tile->id == Tile::greenWeb_Id || tile->id == Tile::redStoneDust_Id || tile->id == Tile::tripWire_Id;
}

float ShearsItem::getDestroySpeed(shared_ptr<ItemInstance> itemInstance, Tile *tile)
{
	if (tile->id == Tile::web_Id || tile->id == Tile::greenWeb_Id || tile->id == Tile::leaves_Id)
	{
		return 15;
	}
