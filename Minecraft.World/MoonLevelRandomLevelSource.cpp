...
const double MoonLevelRandomLevelSource::SNOW_SCALE = 0.3;
const double MoonLevelRandomLevelSource::SNOW_CUTOFF = 0.5;

MoonLevelRandomLevelSource::MoonLevelRandomLevelSource(Level* level, int64_t seed, bool generateStructures) : generateStructures(generateStructures)
{
...
MoonLevelRandomLevelSource::~MoonLevelRandomLevelSource()
{
	delete caveFeature;
...
int g_numPrepareHeightCallsA = 0;
LARGE_INTEGER g_totalPrepareHeightsTimeA = { 0,0 };
LARGE_INTEGER g_averagePrepareHeightsTimeA = { 0, 0 };
...
int MoonLevelRandomLevelSource::getMinDistanceToEdge(int xxx, int zzz, int worldSize, float falloffStart)
{
...
float MoonLevelRandomLevelSource::getHeightFalloff(int xxx, int zzz, int* pEMin)
{
	///////////////////////////////////////////////////////////////////
...
void MoonLevelRandomLevelSource::prepareHeights(int xOffs, int zOffs, byteArray blocks)
{
				...
				for (int y = 0; y < CHUNK_HEIGHT; y++)
				{
					...
					for (int x = 0; x < CHUNK_WIDTH; x++)
					{
						...
						for (int z = 0; z < CHUNK_WIDTH; z++)
						{
							...
							if ((val += vala) > comp)
							{
								tileId = static_cast<byte>(Tile::moonStone_Id);
							}
							...
							if (emin == 0)
							{
								...
								if (yc * CHUNK_HEIGHT + y <= (level->getSeaLevel() - 10)) tileId = Tile::moonStone_Id;
								else if (yc * CHUNK_HEIGHT + y < level->getSeaLevel()) tileId = 0;
							}
	...
	g_numPrepareHeightCallsA++;
	g_totalPrepareHeightsTimeA.QuadPart += timeInFunc.QuadPart;
	g_averagePrepareHeightsTimeA.QuadPart = g_totalPrepareHeightsTimeA.QuadPart / g_numPrepareHeightCallsA;
...
void MoonLevelRandomLevelSource::buildSurfaces(int xOffs, int zOffs, byteArray blocks, BiomeArray biomes)
{
	int waterHeight = level->seaLevel;
				...
				else
				{
					...
					else if (old == Tile::moonStone_Id)
					{
						if (run == -1)
						{
							...
							{
								top = 0;
								material = static_cast<byte>(Tile::moonStone_Id);
							}
...
LevelChunk* MoonLevelRandomLevelSource::create(int x, int z)
{
	return getChunk(x, z);
}

LevelChunk* MoonLevelRandomLevelSource::getChunk(int xOffs, int zOffs)
{
...
void MoonLevelRandomLevelSource::lightChunk(LevelChunk* lc)
{
	lc->recalcHeightmap();
}


doubleArray MoonLevelRandomLevelSource::getHeights(doubleArray buffer, int x, int y, int z, int xSize, int ySize, int zSize, BiomeArray& biomes)
{
...
bool MoonLevelRandomLevelSource::hasChunk(int x, int y)
{
	return true;
}

void MoonLevelRandomLevelSource::calcWaterDepths(ChunkSource* parent, int xt, int zt)
{
	int xo = xt * 16;
	int zo = zt * 16;
	for (int x = 0; x < 16; x++)
			...
			if (h <= 0)
			{
				if (level->getHeightmap(xp - 1, zp) > 0 || level->getHeightmap(xp + 1, zp) > 0 || level->getHeightmap(xp, zp - 1) > 0 || level->getHeightmap(xp, zp + 1) > 0)
				{
					...
					if (hadWater)
					{
						...
						if (hadWater)
						{
							level->setTileAndData(xp, y, zp, 0, 7, Tile::UPDATE_CLIENTS);
							for (int y2 = 0; y2 < y; y2++)
							{
								level->setTileAndData(xp, y2, zp, 0, 8, Tile::UPDATE_CLIENTS);
							}
						}
					}
				}
...
void MoonLevelRandomLevelSource::postProcess(ChunkSource* parent, int xt, int zt)
{
...
bool MoonLevelRandomLevelSource::save(bool force, ProgressListener* progressListener)
{
	return true;
}

bool MoonLevelRandomLevelSource::tick()
{
	return false;
}

bool MoonLevelRandomLevelSource::shouldSave()
{
	return true;
}

wstring MoonLevelRandomLevelSource::gatherStats()
{
	return L"MoonLevelRandomLevelSource";
}

vector<Biome::MobSpawnerData*>* MoonLevelRandomLevelSource::getMobsAt(MobCategory* mobCategory, int x, int y, int z)
{
...
TilePos* MoonLevelRandomLevelSource::findNearestMapFeature(Level* level, const wstring& featureName, int x, int y, int z)
{
...
void MoonLevelRandomLevelSource::recreateLogicStructuresForChunk(int chunkX, int chunkZ)
{
	if (generateStructures)
	{
