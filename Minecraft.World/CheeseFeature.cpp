...
CheeseFeature::CheeseFeature(int radius)
{
	this->tile = Tile::cheeseOre_Id;
	this->radius = radius;
}

bool CheeseFeature::place(Level* level, Random* random, int x, int y, int z)
{
	...
	for (int xx = x - r; xx <= x + r; xx++)
	{
		for (int zz = z - r; zz <= z + r; zz++)
		{
			...
			for (int yy = y - yr; yy <= y + yr; yy++)
			{
				...
				if (t == Tile::moonDirt_Id || t == Tile::cheeseBlock_Id)
				{
