...
SpaceShapedRecipy::SpaceShapedRecipy(int width, int height, ItemInstance** recipeItems, ItemInstance* result, int iGroup)
	: resultId(result->id)
{
	this->width = width;
	this->height = height;
...
const int SpaceShapedRecipy::getGroup()
{
	return group;
}

const ItemInstance* SpaceShapedRecipy::getResultItem()
{
	return result;
}

bool SpaceShapedRecipy::matches(shared_ptr<CraftingContainer> craftSlots, Level* level)
{
...
bool SpaceShapedRecipy::matches(shared_ptr<CraftingContainer> craftSlots, int xOffs, int yOffs, bool xFlip)
{
	for (int x = 0; x < 3; x++) {
		for (int y = 0; y < 3; y++) {
			int xs = x - xOffs;
			...
			if (expected->getAuxValue() != SpaceRecipes::ANY_AUX_VALUE && expected->getAuxValue() != item->getAuxValue())
			{
				return false;
			}
		}
...
shared_ptr<ItemInstance> SpaceShapedRecipy::assemble(shared_ptr<CraftingContainer> craftSlots)
{
...
int SpaceShapedRecipy::size()
{
	return width * height;
}

// 4J-PB
bool SpaceShapedRecipy::reqs(int iRecipe)
{
	app.DebugPrintf("SpaceShapedRecipy %d\n", iRecipe);
	int iCount = 0;
	for (int x = 0; x < 3; x++)
	{
		for (int y = 0; y < 3; y++)
...
void SpaceShapedRecipy::reqs(INGREDIENTS_REQUIRED* pIngReq)
{
	//printf("SpaceShapedRecipy %d\n",iRecipe);
	...
	TempIngReq.iIngC = 0;
	TempIngReq.iType = RECIPE_TYPE_3x3; 
	// 3x3
	...
	memset(TempIngReq.iIngAuxValA, SpaceRecipes::ANY_AUX_VALUE, sizeof(int) * 9);
		...
		for (int y = 0; y < 3; y++)
		{
			if (x < width && y < height)
			{
				...
				if (expected != nullptr)
				{
					...
					{
						if ((TempIngReq.iIngIDA[j] == expected->id) && (iAuxVal == SpaceRecipes::ANY_AUX_VALUE || TempIngReq.iIngAuxValA[j] == iAuxVal))
						{
							bFound = true;
							break;
						}
...
SpaceShapedRecipy* SpaceShapedRecipy::keepTag()
{
	_keepTag = true;
	return this;
}