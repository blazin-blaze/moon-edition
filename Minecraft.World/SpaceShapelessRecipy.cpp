...
SpaceShapelessRecipy::SpaceShapelessRecipy(ItemInstance* result, vector<ItemInstance*>* ingredients, _eGroupType egroup) :
	result(result),
...
const int SpaceShapelessRecipy::getGroup()
{
	return group;
}

const ItemInstance* SpaceShapelessRecipy::getResultItem()
{
	return result;
}

bool SpaceShapelessRecipy::matches(shared_ptr<CraftingContainer> craftSlots, Level* level)
{
	...
	for (int y = 0; y < 3; y++)
	{
		for (int x = 0; x < 3; x++)
		{
			...
			if (item)
			{
				...
				{
					if (item->id == ingredient->id && (ingredient->getAuxValue() == SpaceRecipes::ANY_AUX_VALUE || item->getAuxValue() == ingredient->getAuxValue()))
					{
						found = true;
...
shared_ptr<ItemInstance> SpaceShapelessRecipy::assemble(shared_ptr<CraftingContainer> craftSlots)
{
	return result->copy();
}

int SpaceShapelessRecipy::size()
{
	return static_cast<int>(ingredients->size());
}

// 4J-PB
bool SpaceShapelessRecipy::reqs(int iRecipe)
{
	vector <ItemInstance*>* tempList = new vector<ItemInstance*>;

	*tempList = *ingredients;

	//printf("SpaceShapelessRecipy %d\n",iRecipe);
...
void SpaceShapelessRecipy::reqs(INGREDIENTS_REQUIRED* pIngReq)
{
	int iCount = 0;
	bool bFound;
	int j;
	...
	TempIngReq.iIngC = 0;
	TempIngReq.iType = RECIPE_TYPE_3x3; // all the dyes can be made in a 2x2
