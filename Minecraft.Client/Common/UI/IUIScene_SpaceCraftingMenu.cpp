...
#include "IUIScene_SpaceCraftingMenu.h"

#include "UI.h"

/*Recipy::_eGroupType IUIScene_SpaceCraftingMenu::m_GroupTypeMapping4GridA[IUIScene_SpaceCraftingMenu::m_iMaxGroup2x2] =
{
...
SpaceRecipy::_eGroupType IUIScene_SpaceCraftingMenu::m_GroupTypeMapping9GridA[IUIScene_SpaceCraftingMenu::m_iMaxGroup3x3] =
{
	SpaceRecipy::eGroupType_Rocket,
	SpaceRecipy::eGroupType_Compressed,
	SpaceRecipy::eGroupType_Accessory,
	SpaceRecipy::eGroupType_Space_Misc
};


LPCWSTR IUIScene_SpaceCraftingMenu::m_GroupIconNameA[m_iMaxGroup3x3] =
{
	L"Space Transportation",//Recipy::eGroupType_Structure,
	L"Compressed Metals",//Recipy::eGroupType_Tool,
	L"Space Accessories",//Recipy::eGroupType_Food,
	L"Miscellaneous",//Recipy::eGroupType_Armour
};

/*IUIScene_SpaceCraftingMenu::_eGroupTab IUIScene_SpaceCraftingMenu::m_GroupTabBkgMapping2x2A[m_iMaxGroup2x2] =
{
	eGroupTab_Left,
	eGroupTab_Middle,
	eGroupTab_Middle,
...
IUIScene_SpaceCraftingMenu::_eGroupTab IUIScene_SpaceCraftingMenu::m_GroupTabBkgMapping3x3A[m_iMaxGroup3x3] =
{
	eGroupTab_Left,
	eGroupTab_Middle,
	eGroupTab_Middle,
...
IUIScene_SpaceCraftingMenu::IUIScene_SpaceCraftingMenu()
{
...
LPCWSTR IUIScene_SpaceCraftingMenu::GetGroupNameText(int iGroupType)
{
	switch (iGroupType)
	{
	case SpaceShapedRecipy::eGroupType_Rocket:
		return app.GetString(IDS_GROUPNAME_ROCKET);
	case SpaceShapedRecipy::eGroupType_Compressed:
		return app.GetString(IDS_GROUPNAME_COMPRESSED);
	case SpaceShapedRecipy::eGroupType_Accessory:
		return app.GetString(IDS_GROUPNAME_ACCESSORY);
	case SpaceShapedRecipy::eGroupType_Space_Misc:
		return app.GetString(IDS_GROUPNAME_SPACE_MISC);
	default:
		return app.GetString(IDS_GROUPNAME_SPACE_MISC);
	}
}

bool IUIScene_SpaceCraftingMenu::handleKeyDown(int iPad, int iAction, bool bRepeat)
{
	bool bHandled = false;
		...
		if (m_pPlayer && m_pPlayer->inventory)
		{
			...
			if (app.DebugSettingsOn() && app.GetGameSettingsDebugMask(ProfileManager.GetPrimaryPad()) & (1L << eDebugSetting_CraftAnything))
			{
				if (CanBeMadeA[m_iCurrentSlotHIndex].iCount != 0)
				{
					...
					pMinecraft->localgameModes[iPad]->handleSpaceCraftItem(iRecipe, m_pPlayer);
					...
					if (pTempItemInst->id != Item::fireworksCharge_Id && pTempItemInst->id != Item::fireworks_Id)
					{
						...
						for (int i = 0; i < pRecipeIngredientsRequired[iRecipe].iIngC; i++)
						{
							for (int j = 0; j < pRecipeIngredientsRequired[iRecipe].iIngValA[i]; j++)
							{
								...
								if (pRecipeIngredientsRequired[iRecipe].iIngAuxValA[i] != SpaceRecipes::ANY_AUX_VALUE)
								{
...
//
//////////////////////////////////////////////////////////////////////////
void IUIScene_SpaceCraftingMenu::CheckRecipesAvailable()
{
	...
	if (m_pPlayer && m_pPlayer->inventory)
	{
		...
		*/
		SpaceRecipyList* recipes = ((SpaceRecipes*)SpaceRecipes::getInstance())->getRecipies();
		SpaceRecipy::INGREDIENTS_REQUIRED* pRecipeIngredientsRequired = SpaceRecipes::getInstance()->getRecipeIngredientsArray();
				...
				for (unsigned int k = 0; k < m_pPlayer->inventory->items.length; k++)
				{
					if (m_pPlayer->inventory->items[k] != nullptr)
					{
						...
						if ((m_pPlayer->inventory->items[k]->id == pRecipeIngredientsRequired[i].iIngIDA[j]) &&
							...
							((pRecipeIngredientsRequired[i].iIngAuxValA[j] == SpaceRecipes::ANY_AUX_VALUE) || (pRecipeIngredientsRequired[i].iIngAuxValA[j] == m_pPlayer->inventory->items[k]->getAuxValue()))
							)
						{
								...
								for (unsigned int l = 0; l < m_pPlayer->inventory->items.length; l++)
								{
									if (m_pPlayer->inventory->items[l] != nullptr)
									{
										if (
											...
											((pRecipeIngredientsRequired[i].iIngAuxValA[j] == SpaceRecipes::ANY_AUX_VALUE) || (pRecipeIngredientsRequired[i].iIngAuxValA[j] == m_pPlayer->inventory->items[l]->getAuxValue()))
											)
										{
				...
				if (bFoundA[j] == false)
				{
					...
					while (iMissing != 0)
					{
						...
						if (((pRecipeIngredientsRequired[i].uiGridA[iGridIndex] & 0x00FFFFFF) == pRecipeIngredientsRequired[i].iIngIDA[j]) &&
							((pRecipeIngredientsRequired[i].iIngAuxValA[j] == SpaceRecipes::ANY_AUX_VALUE) || (pRecipeIngredientsRequired[i].iIngAuxValA[j] == ((pRecipeIngredientsRequired[i].uiGridA[iGridIndex] & 0xFF000000) >> 24))))
						{
...
//
//////////////////////////////////////////////////////////////////////////
void IUIScene_SpaceCraftingMenu::UpdateHighlight()
{
...
//
//////////////////////////////////////////////////////////////////////////
void IUIScene_SpaceCraftingMenu::UpdateVerticalSlots()
{
	//RecipyList *recipes = ((Recipes *)Recipes::getInstance())->getRecipies();
	SpaceRecipy::INGREDIENTS_REQUIRED* pRecipeIngredientsRequired = SpaceRecipes::getInstance()->getRecipeIngredientsArray();
...
//
//////////////////////////////////////////////////////////////////////////
void IUIScene_SpaceCraftingMenu::DisplayIngredients()
{
	//RecipyList *recipes = ((Recipes *)Recipes::getInstance())->getRecipies();
	SpaceRecipy::INGREDIENTS_REQUIRED* pRecipeIngredientsRequired = SpaceRecipes::getInstance()->getRecipeIngredientsArray();
...
//
//////////////////////////////////////////////////////////////////////////
void IUIScene_SpaceCraftingMenu::UpdateDescriptionText(bool bCanBeMade)
{
	int iIDSString = 0;
	//RecipyList *recipes = ((Recipes *)Recipes::getInstance())->getRecipies();
	SpaceRecipy::INGREDIENTS_REQUIRED* pRecipeIngredientsRequired = SpaceRecipes::getInstance()->getRecipeIngredientsArray();
...
//
//////////////////////////////////////////////////////////////////////////
void IUIScene_SpaceCraftingMenu::UpdateTooltips()
{
	//RecipyList *recipes = ((Recipes *)Recipes::getInstance())->getRecipies();
	SpaceRecipy::INGREDIENTS_REQUIRED* pRecipeIngredientsRequired = SpaceRecipes::getInstance()->getRecipeIngredientsArray();
	// Update tooltips
...
void IUIScene_SpaceCraftingMenu::HandleInventoryUpdated()
{
...
bool IUIScene_SpaceCraftingMenu::isItemSelected(int itemId)
{
